...
public interface GoalRepository extends JpaRepository<GoalEntity, Long> {
    ...
    @Query("select new com.example.demo.src.goal.model.GetGoalRes(m.id, m.image, m.goal_amount, m.amount,m.init_amount,m.category_name, m.date) from GoalEntity m where m.user_id.id = :userIdx")
    List<GetGoalRes> findGoalList(@Param("userIdx") Long userIdx);

    @Query("select new com.example.demo.src.goal.model.GetGoalRes(m.id, m.image, m.goal_amount, m.amount,m.init_amount, m.category_name, m.date) from GoalEntity m where m.user_id.id = :userIdx and m.id = :goalIdx")
    GetGoalRes findGoal(@Param("userIdx") Long userIdx, @Param("goalIdx") Long goalIdx);

    //  @Query("select m from GoalEntity m left join f")

    @Query("select new com.example.demo.src.goal.model.GetGoalRes(m.id, m.image, m.goal_amount, m.amount,m.init_amount, m.category_name, m.date) from GoalEntity m where m.user_id.id = :userIdx order by m.id asc")
    List<GetGoalRes> findGoalListByAsc(@Param("userIdx") Long userIdx);

    @Query("select new com.example.demo.src.goal.model.GetGoalRes(m.id, m.image, m.goal_amount, m.amount,m.init_amount, m.category_name, m.date) from GoalEntity m where m.user_id.id = :userIdx order by m.id desc")
    ...
    void updateGoal(@Param("goalAmount") int goalAmount, @Param("initAmount") int initAmount, @Param("category_name") String category_name, @Param("userEntity") UserEntity userEntity, @Param("goalIdx") Long goalIdx, @Param("goalImage") String goalImage);
